...
- `llm_follow_astrbot_persona`: 是否继承 AstrBot 当前默认人格
- `llm_prompt`: 自定义补充提示词；继承人格开启时作为补充约束，关闭时作为主要人格提示词

默认逻辑已经改成通用方案：优先继承 AstrBot 当前默认人格，插件自己只补充“这是一次喂食互动，回答要简短直接，不解释规则”这类场景约束。这样不需要每次都来改插件代码，平时直接改 AstrBot 的人格，或者在插件面板里补一段 `llm_prompt` 就够了。
